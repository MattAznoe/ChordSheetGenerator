...
{   
    ...
    private AppSettings _workingSettings = null;
    ...
    {
        for (int i = 0; i < Constants._printFontTypeCount; i++)
        {
            _appSettings.Fonts [i] = _settings.GetFont ((PrintFontType)i);
        }

        _appSettings.DefaultFolder = _settings.DefaultFolder;
        _appSettings.DefaultUseSharps = _settings.DefaultUseSharps;
        ...
        chkUseSharps.Active = _appSettings.DefaultUseSharps;
    }
    ...
    protected void SaveSettings()
    {
        for (int i = 0; i < Constants._printFontTypeCount; i++)
        {
            _settings.SetFont (_appSettings.Fonts [i], (PrintFontType)i);
        }
        ...
        _settings.DefaultUseSharps = _appSettings.DefaultUseSharps;
    ...
    {
        CSGen.SettingsDialog dialog = new CSGen.SettingsDialog (_appSettings);
    ...
    {
        _workingSettings = ((CSGen.SettingsDialog)sender).WorkingAppSettings;
        drawingArea.QueueDraw();
    }
    ...
    protected void on_btnTransposeDown_clicked (object sender, EventArgs e)
    {
        ...
        if (txtEditor.Buffer.GetSelectionBounds (out start, out end))
        {
            ...
            text = _currentSong.ContentText = FileOperations.Transpose (text, TransposeDirection.Down, 1, chkUseSharps.Active);
        ...
        } else
        {
            ...
            _currentSong.ContentText = FileOperations.Transpose (_currentSong.ContentText, TransposeDirection.Down, 1, chkUseSharps.Active);
    ...
    protected void on_btnTransposeUp_clicked (object sender, EventArgs e)
    {
        ...
        if (txtEditor.Buffer.GetSelectionBounds (out start, out end))
        {
            // Only transpose the selected text.
            string text = txtEditor.Buffer.GetText (start, end, true);
            text = _currentSong.ContentText = FileOperations.Transpose (text, TransposeDirection.Up, 1, chkUseSharps.Active);
        ...
        {
            _currentSong.ContentText = FileOperations.Transpose (_currentSong.ContentText, TransposeDirection.Up, 1, chkUseSharps.Active);
            txtEditor.Buffer.Text = _currentSong.ContentText;
        }
    }
    ...
    protected void onChkUseSharps (object sender, EventArgs e)
    {
        NormalizeChordTypes ();
    }
