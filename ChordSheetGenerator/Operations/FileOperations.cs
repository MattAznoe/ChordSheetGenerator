...
namespace CSGen.Operations
{
	public static class FileOperations
	{
		...
		public static string Transpose(string text, TransposeDirection direction, int steps)
		{
			// Get all of the chords in the song.
			MatchCollection mc = Constants._reChord.Matches (text);

			// Determine the chord progression to use.
			tx_Progression = Constants._chordProgressionSharp;
			foreach (Match m in mc)
			{
				if ((m.Groups [1].Length == 2 && m.Groups [1].ToString ().EndsWith ("b"))
				    || (m.Groups [5].Length == 2 && m.Groups [5].ToString ().EndsWith ("b")))
				{
